...
这个 Skill 的定位很单纯：凡是关于 Codex 本身的问题，它都先去官方索引里找到对应页面，抓取当前内容，再基于真实文档作答。它覆盖 Codex CLI（安装、slash 命令、命令行选项、配置、sandbox、认证）、IDE 扩展、Codex app（自动化、worktrees、computer use、Chrome 扩展）、Codex cloud，以及 subagents、skills、MCP、企业版、GitHub/Linear/Slack 集成等概念 and 场景。
...
第一步是读索引。它用 `read_url_content` 把 `llms.txt` 整个抓下来，要求返回未经改动的原始 Markdown，保留每一条 `- [Title](URL): description` 以及分节标题。这一步不能跳过，哪怕你觉得自己记得目标 URL。因为文档的 slug 会被重命名，索引才是唯一可信的真相来源。这里有一个刻意的设计取舍：索引一次性全文加载，直接在整张地图上搜，而不是先按分节做粗筛。分节只是页面的排版方式，不构成跳过某些条目的理由，粗筛反而容易漏掉恰好落在你没看的那一节里的答案。
...
第三步是抓取这一批。对每个选中的 URL 调用 `read_url_content`。
...
flowchart TD
    ...
    C --> D[read_url_content 抓取 codex/slug.md 原始 Markdown]
    D --> E{信息够回答了吗?}
    E -->|够| F[基于真实文档作答并附出处]
    E -->|不够, 累计 < 9 页| C
