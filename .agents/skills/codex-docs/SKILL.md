...
Call the `read_url_content` tool to fetch the index:
```
read_url_content Url=https://developers.openai.com/codex/llms.txt
```

The index is a single map of ~200 lines, grouped under `## Section` headers, each entry in the form `- [Title](https://developers.openai.com/codex/<slug>.md): description`. URLs end in `.md` — every page has a Markdown twin at `/codex/<slug>.md`, so the targets are raw Markdown.
...
For each chosen URL, fetch its content using `read_url_content`:

```
read_url_content Url=<URL from index>
```
