...
- **索引是一个本地文件** `references/docs-manifest.json`，由配套的 `antigravity-docs-index-builder` Skill 生成。这个 Skill 直接通过 `view_file` 读取它，**不去抓 `llms.txt`，也不爬网站**。
- **正文在另一个 `.md` 地址**（manifest 每条里的 `content_url`，位于 `/assets/docs/...` 下）。这个地址是能抓的，也正是它用 `read_url_content` 去抓的东西。

So the flow is: read the local manifest → pick pages → fetch their `content_url` using `read_url_content`. 这个"索引和正文分离"的设计，是 Antigravity 的 SPA 特性逼出来的，也是这个 Skill 最不一样的地方。
...
第一步是读 manifest。它用 `view_file` 读那个本地 JSON，里面有 `_meta`（生成日期、源 bundle）和 `pages`，每条是 `{section, slug, title, description, content_url}`。如果文件不存在，就提示用户先跑 `antigravity-docs-index-builder`，而不是自己去猜 URL。
...
第三步是抓这一批。对每个选中的页面，用 `read_url_content` 抓它的 `content_url`（那个 `/assets/docs/....md` 地址）。
...
```mermaid
flowchart TD
    A[用户提问] --> B[view_file 本地 docs-manifest.json]
    B --> C[按 title+description 匹配挑 1 到 3 页]
    C --> D[read_url_content 对应的 assets/docs/*.md]
    D --> E{信息够回答了吗?}
