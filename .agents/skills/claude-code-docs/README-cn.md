...
第一步是读索引。它用 `read_url_content` 把 `llms.txt` 整个抓下来，要求返回未经改动的原始 Markdown，保留每一条 `- [Title](URL): description`。这一步不能跳过，哪怕你觉得自己记得目标 URL。因为文档的 slug 会被重命名，索引才是唯一可信的真相来源。
...
flowchart TD
    ...
    C --> D[read_url_content 抓取这一批原始 Markdown]
    D --> E{信息够回答了吗?}
    E -->|够| F[基于真实文档作答并附出处]
    E -->|不够, 累计 < 9 页| C
