---
name: port-antigravity-to-claude-code-checker
description: Read-only review of how completely a project's Antigravity configuration has been ported to Claude Code. Scans both sides, compares them against the coding-agent-concept-mapping knowledge base, and writes a gap report to tmp/review-port-antigravity-to-claude-code.md. Makes no changes to the project config. Use to audit a Antigravity to Claude Code migration.
argument-hint: [project path]
allowed-tools: Read, Write, Glob, Grep, Skill
---

# Check port from Antigravity to Claude Code

Audits a project migration from Antigravity to Claude Code without
changing anything. It runs the same scan and mapping logic as the
`port-antigravity-to-claude-code` skill, but instead of writing target files it records what
...
and only write is the review report at `tmp/review-port-antigravity-to-claude-code.md` inside
...
Antigravity and Claude Code primary file or location from its table. This
...
Antigravity artifact and the Claude Code artifact the index named, using
...
Write `tmp/review-port-antigravity-to-claude-code.md` inside the project, overwriting any
existing copy. Use this structure:

```
# Port review: Antigravity to Claude Code
...
Point the user at `tmp/review-port-antigravity-to-claude-code.md` and give a one line summary
...
- **Read only. Never modify project config.** The single allowed write is the
  report file.
- **The report path is fixed:** `tmp/review-port-antigravity-to-claude-code.md`, overwritten
  each run.
