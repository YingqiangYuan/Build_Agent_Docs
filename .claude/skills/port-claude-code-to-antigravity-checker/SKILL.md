...
# Check port from Claude Code to Antigravity

Audits a project migration from Claude Code to Antigravity without
changing anything. It runs the same scan and mapping logic as the
`port-claude-code-to-antigravity` skill, but instead of writing target files it records what
...
and only write is the review report at `tmp/review-port-claude-code-to-antigravity.md` inside
...
Claude Code artifact and the Antigravity artifact the index named, using
...
Write `tmp/review-port-claude-code-to-antigravity.md` inside the project, overwriting any
existing copy. Use this structure:

```
# Port review: Claude Code to Antigravity
...
Point the user at `tmp/review-port-claude-code-to-antigravity.md` and give a one line summary
