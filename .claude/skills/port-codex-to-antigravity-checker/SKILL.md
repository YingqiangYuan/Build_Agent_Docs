---
name: port-codex-to-antigravity-checker
description: Read-only review of how completely a project's Codex configuration has been ported to Antigravity. Scans both sides, compares them against the coding-agent-concept-mapping knowledge base, and writes a gap report to tmp/review-port-codex-to-antigravity.md. Makes no changes to the project config. Use to audit a Codex to Antigravity migration.
argument-hint: [project path]
allowed-tools: Read, Write, Glob, Grep, Skill
---

# Check port from Codex to Antigravity

Audits a project migration from Codex to Antigravity without
changing anything. It runs the same scan and mapping logic as the
`port-codex-to-antigravity` skill, but instead of writing target files it records what
...
and only write is the review report at `tmp/review-port-codex-to-antigravity.md` inside
...
Codex and Antigravity primary file or location from its table. This
...
Codex artifact and the Antigravity artifact the index named, using
...
Antigravity specifics. Classify each concept as fully ported, partially
ported, or missing on the target side.

### 5. Write the report

Write `tmp/review-port-codex-to-antigravity.md` inside the project, overwriting any
existing copy. Use this structure:

```
# Port review: Codex to Antigravity
...
Point the user at `tmp/review-port-codex-to-antigravity.md` and give a one line summary
...
- **Read only. Never modify project config.** The single allowed write is the
  report file.
- **The report path is fixed:** `tmp/review-port-codex-to-antigravity.md`, overwritten
  each run.
...
- **Ground target details in `antigravity-docs`.** Do not judge a gap from
  memory of how Antigravity works.
