---
name: port-claude-code-to-codex-checker
description: Read-only review of how completely a project's Claude Code configuration has been ported to Codex. Scans both sides, compares them against the coding-agent-concept-mapping knowledge base, and writes a gap report to tmp/review-port-claude-code-to-codex.md. Makes no changes to the project config. Use to audit a Claude Code to Codex migration.
argument-hint: [project path]
allowed-tools: Read, Write, Glob, Grep, Skill
---

# Check port from Claude Code to Codex

Audits a project migration from Claude Code to Codex without
changing anything. It runs the same scan and mapping logic as the
`port-claude-code-to-codex` skill, but instead of writing target files it records what
...
and only write is the review report at `tmp/review-port-claude-code-to-codex.md` inside
...
- **Concept roster plus each agent's primary file or location:**
  ...
  scope. Its per concept table names the Claude Code and Codex
  file or location.
...
Claude Code and Codex primary file or location from its table. This
...
Claude Code artifact and the Codex artifact the index named, using
...
Write `tmp/review-port-claude-code-to-codex.md` inside the project, overwriting any
existing copy. Use this structure:

```
# Port review: Claude Code to Codex
...
Point the user at `tmp/review-port-claude-code-to-codex.md` and give a one line summary
...
- **Read only. Never modify project config.** The single allowed write is the
  report file.
- **The report path is fixed:** `tmp/review-port-claude-code-to-codex.md`, overwritten
  each run.
