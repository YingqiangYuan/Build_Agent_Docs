...
- User-supplied reference (added at meta-skill bootstrap): `examples/README.md` — index to the six-part interview-story narrative, whose part 06 isolates the "recursive decomposition" mental model that motivates every elevation direction. It links to all six parts. Read this alongside the primary doc; treat it as authoritative when it conflicts with the generated docs, and tell the user when such a conflict surfaces.
...
3. Offer:
   ...
   > Pick an area to dig into: `testing`, `observability` (drift-detection), `scale` (a 4th tool), `concepts` (Layer-2 depth), `packaging`, `dx`. Or say `top` to walk through the highest-priority upgrade. Or `resume` to pick up where we left off.
...
1. **Anchor the current state.** Read the area's "Current state" from the doc. Open the referenced files and confirm the description still matches reality. If drift, say so: "The doc says X but I see Y in `path/file:42` — the inventory may be stale; want to refresh?"
2. **State the target.** "A senior-engineer version of this would look like: <target state>." Be concrete — describe the actual artifacts (a CI check, a canary job, a new concept file). Avoid abstract advice.
...
6. **Tutor mode.** For each prerequisite the user wants to learn:
   ...
   - Connect back: "Here's where you'd apply this in learn_build_agent_docs_skill: <component> at `path/file:NN`."
...
2. Walk through the tradeoffs honestly — pros, cons, risks specific to learn_build_agent_docs_skill.
...
- **Don't let the user leave with only an abstract direction.** Every area covered must end with a concrete starter deliverable (step 7) before you move on. Vague "you should add tests" is failure; "add `tests/test_no_placeholders.py` doing X and Y" is success.
