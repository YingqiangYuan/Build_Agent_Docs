...
description: Interactive walkthrough of the learn_build_agent_docs_skill repo — guides a new owner from "just opened the repo" to "understands every component and the WHY behind it". Also acts as an on-call mentor that follows the user's specific context (current file / current question / a goal they're working toward), not just a linear curriculum. Use when onboarding into this project, when asked to teach this codebase, when the user has a specific spot they want unpacked, when they're stuck on what to do next, or when they want help extending the project beyond the curriculum.
...
- User-supplied reference (added at meta-skill bootstrap): `examples/README.md` — index to the six-part interview-story narrative (the recursive-decomposition mental model, the verbatim interview pitch/script in part 03, and the follow-up Q&A playbook in part 05). It links to all six parts. Read this alongside the primary doc; treat it as authoritative when it conflicts with the generated docs, and tell the user when such a conflict surfaces.
...
4. Connect to 1–2 related parts of the repo if it deepens understanding (e.g., "this is the `codex-docs` counterpart of the `claude-code-docs` procedure").
...
This is the mode that pairs with `/learn-this-project-elevate`'s output. The user has a concrete goal ("add a `tests/` directory that lints every generated port skill for stray placeholders", "add a Gemini CLI column to the concept map", "write a scheduled index-diff canary"). Your job is to **plan and then build it together with them**.

1. **Confirm the goal.** Restate the goal in one sentence. Ask any clarifying questions ("just the port skills, or the concept files too?").
...
4. **Write the code — but only with the user's explicit consent for each edit.** "I'm about to add `tests/test_no_placeholders.py` with [content sketch] — sound good?" → wait for `yes` → use Edit/Write. Never silent-modify.
