---
name: port-antigravity-to-claude-code
description: Port a project's Antigravity configuration to Claude Code, keeping the Antigravity files in place. Scans the project for Antigravity config artifacts, maps each to its Claude Code equivalent using the concept-mapping knowledge base, and creates or edits the matching Claude Code files. Use when migrating a repo from Antigravity to Claude Code.
...
Migrates one project's agent configuration from Antigravity (the source) to
Claude Code (the target). This is a project level port: it touches only files
inside the given project, never global or machine level config. Source files are
preserved. The Claude Code config is created alongside them, not on top of
...
- **Concept roster plus each agent's primary file or location:**
  ...
  scope. Its per concept table names the Antigravity and Claude Code
...
Antigravity primary file or location and the Claude Code primary file or
location from its table. This roster is never hardcoded; it comes from the index
every run.

### 3. Scan for source artifacts

For each concept in the roster, look inside the project for the Antigravity
...
Claude Code. When the detail file is thin on a target specific point, consult
the `claude-code-docs` skill for the current Claude Code file format and
field names, and `antigravity-docs` for the source. Do not migrate from
memory; these tools change often.

### 5. Execute the migration

Create or edit the Claude Code file or files named by the mapping,
translating paths, filenames, formats, and field names per the concept detail.
Follow the Claude Code porting-in notes exactly, since that is where the
friction lives (renamed files, changed field keys, format shifts such as JSON to
TOML). Preserve every Antigravity file untouched: the target config coexists
...
- **Ground target details in `claude-code-docs`.** When the mapping is thin,
